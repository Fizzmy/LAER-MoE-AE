...
CHECKPOINT_PATH=$BASE_DIR/checkpoints/megatron/$MODEL_NAME
...
if [ $SAVE_OR_LOAD -eq 1 ]; then
    ...
    torchrun ${DISTRIBUTED_ARGS[@]} pretrain_gpt.py \
        ...
        ${TRAINING_ARGS[@]} \
        ${MODEL_PARALLEL_ARGS[@]} \
        ${LOGGING_ARGS[@]} | tee training_log/megatron_${MODEL_NAME}_${DATASET}_batch${BATCH_SIZE}_seq${SEQ_LENGTH}_aux${AUX_LOSS}_${ARNOLD_ID}.log
fi
